...
func (b *Generator) BuildCode(w io.Writer, typeSelector func(name, inherits spec.TypeName) bool) error {
	...
	f.Line()
	f.HeaderComment("Code generated by github.com/andrewkroh/go-ingest-node/internal/generator, DO NOT EDIT.")
	f.PackageComment(`// Package ingestnode provides a data model for Elasticsearch [Ingest Node] 
// pipelines. The model is generated from the [elasticsearch-specification].
//
// [Ingest Node]: https://www.elastic.co/guide/en/elasticsearch/reference/current/processors.html
...
// processorStructGoDoc builds a godoc comment for a processor struct. This is a
// hack to associate the documentation info from the
// `ingest._types.ProcessorContainer` properties to the processors. Ideally, the
// `_ingest.types.*Processor` types would have complete documentation, but they
// do not.
func (b *Generator) processorStructGoDoc(ifc *spec.Interface) string {
	pc, ok := b.allTypes[spec.TypeName{
	...
	// Attempt to find the documentation within the properties of the
	// ProcessorContainer.
	for _, p := range pc.Value.(spec.Interface).Properties {
		if p.Type.TypeName != ifc.TypeName {
			continue
		}
		...
		if p.Description != nil {
			...
			d = wordwrap.WrapString(d, 80)
			d = strings.TrimSpace(d)
			paragraphs = append(paragraphs, d)
		}
		if p.DocURL != nil {
			paragraphs = append(paragraphs, fmt.Sprintf("Reference: [%s processor]", p.Name))
			paragraphs = append(paragraphs, fmt.Sprintf("[%s processor]: %s", p.Name, *p.DocURL))
		}
...
func (b *Generator) goStruct(ifc *spec.Interface, f *jen.File) error {
	// Special handling to add docs to processor structs.
	if c := b.processorStructGoDoc(ifc); c != "" {
		f.Comment(c)
	}
