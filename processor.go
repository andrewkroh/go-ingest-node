...
// Package ingestnode provides a data model for Elasticsearch [Ingest Node]
...
type FieldAccessPattern string

const (
	ClassicFieldAccessPattern  FieldAccessPattern = "classic"
	FlexibleFieldAccessPattern FieldAccessPattern = "flexible"
)
...
/*
Appends one or more values to an existing array if the field already exists and
it is an array. Converts a scalar to an array and appends one or more values to
it if the field exists and it is a scalar. Creates an array containing the
provided values if the field doesn’t exist. Accepts a single value or an array
of values.
...
/*
The attachment processor lets Elasticsearch extract file attachments in common
formats (such as PPT, XLS, and PDF) by using the Apache text extraction library
Tika.
...
/*
Converts a human readable byte value (for example `1kb`) to its value in bytes
(for example `1024`). If the field is an array of strings, all members of the
array will be converted. Supported human readable units are "b", "kb", "mb",
"gb", "tb", "pb" case insensitive. An error will occur if the field is not a
supported format or resultant value exceeds 2^63.
...
/*
Computes the Community ID for network flow data as defined in the Community ID
Specification. You can use a community ID to correlate network events related to
a single flow.
...
/*
Converts a field in the currently ingested document to a different type, such as
converting a string to an integer. If the field value is an array, all members
will be converted.
...
/*
Extracts fields from CSV line out of a single text field within a document. Any
empty field in CSV will be skipped.
...
type DateIndexNameProcessor struct {
	ProcessorBase
	DateFormats     []string `json:"date_formats,omitempty" yaml:"date_formats,omitempty"`                                           // An array of the expected date formats for parsing dates / timestamps in the document being preprocessed. Can be a java time pattern or one of the following formats: ISO8601, UNIX, UNIX_MS, or TAI64N.
...
/*
Parses dates from fields, and then uses the date or timestamp as the timestamp
for the document.
...
/*
Extracts structured fields out of a single text field by matching the text field
against a delimiter-based pattern.
...
/*
Raises an exception. This is useful for when you expect a pipeline to fail and
want to relay a specific message to the requester.
...
/*
Computes a hash of the document’s content. You can use this hash for content
fingerprinting.
...
/*
Converts geo-grid definitions of grid tiles or cells to regular bounding boxes
or polygons which describe their shape. This is useful if there is a need to
interact with the tile shapes as spatially indexable fields.
...
/*
Converts a string field by applying a regular expression and a replacement. If
the field is an array of string, all members of the array will be converted. If
any non-string values are encountered, the processor will throw an exception.
...
/*
Removes HTML tags from the field. If the field is an array of strings, HTML tags
will be removed from all members of the array.
*/
...
/*
Joins each element of an array into a single string using a separator character
between each element. Throws an error when the field is not an array.
...
type Pipeline struct {
	...
	CreatedDate        *DateTime            `json:"created_date,omitempty" yaml:"created_date,omitempty"`                                                                         // Date and time when the pipeline was created. Only returned if the `human` query parameter is `true`.
	CreatedDateMillis  *int64               `json:"created_date_millis,omitempty" yaml:"created_date_millis,omitempty"`                                                           // Date and time when the pipeline was created, in milliseconds since the epoch.
	ModifiedDate       *DateTime            `json:"modified_date,omitempty" yaml:"modified_date,omitempty"`                                                                       // Date and time when the pipeline was last modified. Only returned if the `human` query parameter is `true`.
	ModifiedDateMillis *int64               `json:"modified_date_millis,omitempty" yaml:"modified_date_millis,omitempty"`                                                         // Date and time when the pipeline was last modified, in milliseconds since the epoch.
	FieldAccessPattern *FieldAccessPattern  `json:"field_access_pattern,omitempty" jsonschema:"enum=classic,enum=flexible,default=classic" yaml:"field_access_pattern,omitempty"` // Controls how processors in this pipeline should read and write data on a document's source.
}
...
document matching the given Grok patterns. The processor can be used to obscure
Personal Identifying Information (PII) by configuring it to detect known
patterns such as email or IP addresses. Text that matches a Grok pattern is
replaced with a configurable string such as `<EMAIL>` where an email address is
matched or simply replace all matches with the text `<REDACTED>` if preferred.
...
(FQDN). Uses the registered domains defined in the Mozilla Public Suffix List.
...
/*
Renames an existing field. If the field doesn’t exist or the new name is already
...
namespace options can’t be set. When the `destination` option is not set, this
processor is in a data stream mode. Note that in this mode, the reroute
...
/*
Splits a field into an array using a separator character. Only works on string
fields.
...
/*
Trims whitespace from a field. If the field is an array of strings, all members
of the array will be trimmed. This only works on leading and trailing
whitespace.
...
an object. This URI object includes properties for the URI’s domain, path,
...
/*
The `user_agent` processor extracts details from the user agent string a browser
sends with its web requests. This processor adds this information by default
under the `user_agent` field.
...
type UserAgentProcessor struct {
	...
	Properties        []UserAgentProperty `json:"properties,omitempty" jsonschema:"default=name,default=os,default=device,default=original,default=version" yaml:"properties,omitempty"` // Controls what properties are added to `target_field`.
